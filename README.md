...
- [Modern C++23 Clang-Format & Static Analysis](#modern-c-23-clang-format-static-analysis) — before and after rule comparison
...
Install `@sebastienrousseau/cpp-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/cpp-config

# pnpm
pnpm add -D @sebastienrousseau/cpp-config

# yarn
yarn add -D @sebastienrousseau/cpp-config

# bun
bun add -d @sebastienrousseau/cpp-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/cpp-config.git
cd cpp-config
make            # check + test
```
...
- **Target environment**: Optimized for LLVM Clang 18+ / C++23.
...
```bash
cp node_modules/@sebastienrousseau/cpp-config/.clang-format ./.clang-format
cp node_modules/@sebastienrousseau/cpp-config/.clang-tidy ./.clang-tidy
cp node_modules/@sebastienrousseau/cpp-config/cmake-format.py ./cmake-format.py
```

### Or point the tool at the installed file

```bash
clang-format --style=file:node_modules/@sebastienrousseau/cpp-config/.clang-format -i src/*.cpp
```
...
`@sebastienrousseau/cpp-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/cpp-config` solves this with three deliberate engineering choices:
...
## Modern C++23 Clang-Format & Static Analysis

Enforces C++ Core Guidelines, concepts, modern templates, and memory safety rules across C++20 and C++23 codebases.
...
### After (@sebastienrousseau/cpp-config Enforced)
...
When maintaining pre-C++11 legacy codebases reliant on proprietary non-standard compiler extensions.
