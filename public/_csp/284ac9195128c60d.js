...
lp=lm?'/cpp-config/'+lm[1]:'/cpp-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/cpp-config/'+lm[1]+'/search-index.json':'/cpp-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
