...
- Added a `native-validate` CI job that installs the real toolchain (clang-format, clang-tidy, cmake-format) and validates the shipped presets.
- Corrected the OpenSSF Best Practices badge, which pointed at another project.
## [0.0.1] - 2026-09-09

### Added

- Initial release of `@sebastienrousseau/cpp-config` conforming to REPO-STANDARD.
- Standardized C++ configuration presets: `.clang-format`, `.clang-tidy`, `cmake-format.py`.
