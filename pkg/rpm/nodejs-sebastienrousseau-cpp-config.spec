...
%global npm_bare      cpp-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-cpp-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable C++ configuration providing standardized clang-format, clang-tidy, and cmake-format rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/cpp-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/cpp-config is a shareable configuration preset with no runtime
...
install -pm 0644 .clang-format %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 .clang-tidy %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 cmake-format.py %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
